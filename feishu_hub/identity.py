"""identity — feishu_hub 唯一身份解析层（user / bot / profile / host）。

设计原则：**lark-cli profile 是身份事实源，feishu_hub 不发明 identity 概念**。
...
feishu_hub 调用方应该用本模块取身份，**不要**从 ``~/.feishu_hub/config.yaml`` 的
