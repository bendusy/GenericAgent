...
def resolve_real_cli(cfg: Dict[str, Any], *, shim_path: str) -> str:
    ...
    if not real:
        raise RuntimeError(
            "feishu_hub: shim.real_lark_cli not configured; "
            "run `python -m feishu_hub init` or set FEISHU_HUB_REAL_LARK_CLI"
        )
    real_resolved = os.path.realpath(real)
    shim_resolved = os.path.realpath(shim_path)
    if real_resolved == shim_resolved:
        raise RuntimeError(
            f"feishu_hub: real_lark_cli ({real}) resolves to shim itself; abort"
        )
    if not os.path.exists(real_resolved):
        raise RuntimeError(f"feishu_hub: real_lark_cli not found: {real}")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        sys.stderr.write(f"[feishu_hub] {e}\n")
        return 127
    ...
    if is_interactive(sub_argv, interactive_verbs):
        if not no_journal:
            try:
                ...
                sys.stderr.write(f"[feishu_hub] journal skipped-write failed: {e}\n")
    ...
    try:
        ...
        sys.stderr.write(f"[feishu_hub] real lark-cli not found: {real_cli}\n")
        return 127
    ...
    if no_journal:
        ...
        try:
            ...
            sys.stderr.write(f"[feishu_hub] journal write failed: {e}\n")
