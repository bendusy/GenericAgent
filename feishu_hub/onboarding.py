...
"我的待办"立即看到 3 条解释 feishu_hub 是什么 / 怎么用的任务，每条都是
一个 agent step 流示范 + 跳链按钮。用户不需要读 README 就能上手。

调用方：``python -m feishu_hub init`` 末尾默认调；``python -m feishu_hub guide``
...
def _welcome_task(user_open_id: str, bot_app_id: str, host: str) -> Dict[str, str]:
    return {
        ...
        "description": (
            ...
            f"- bot:  {bot_app_id}\n"
            f"- host: {host}\n\n"
            "切换身份：`lark-cli profile use <name>` + `python -m feishu_hub whoami` 验证。"
        ),
        "steps": [
            f"已识别 user open_id = {user_open_id}",
            f"已识别 bot app_id = {bot_app_id}（feishu_hub 跟随当前 lark-cli profile，不发明新 identity）",
...
def _try_it_task() -> Dict[str, str]:
    return {
        ...
        "description": (
            ...
            "2. 调 ``python -m feishu_hub.stop_hook``\n"
...
def _base_view_task(base_url: Optional[str]) -> Dict[str, str]:
    base_hint = (
        f"打开 Base：{base_url}\n\n"
        if base_url
        else "Base 看板 URL 没配（config.yaml 缺 bitable.base_token）。请先建好 Base 表，再跑 `python -m feishu_hub indexer migrate-schema`。\n\n"
    )
    return {
        ...
        "description": (
            ...
            "feishu_hub indexer 把 Task 列表反向刷到 Base 索引表，建好的看板/网格/甘特"
            "视图能任意切片：\n\n"
            f"{base_hint}"
            "操作：\n"
            "1. `python -m feishu_hub indexer migrate-schema`（首次：建 6 个新字段）\n"
            "2. `python -m feishu_hub indexer run --full`（全量刷一次）\n"
            "3. 在飞书 Base UI 建看板视图：分组键 = ``Agent`` / ``host`` / ``状态``\n\n"
            "之后 30min 一次 cron 自动增量同步（M3.E）。"
        ),
        "steps": [
            "indexer 触发：`python -m feishu_hub indexer run` (增量) / `--full` (全量校准)",
