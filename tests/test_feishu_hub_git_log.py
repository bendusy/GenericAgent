"""feishu_hub.git_log 单测（用真 git 在 tmp_path 建仓）。"""
...
from feishu_hub import git_log
...
def test_no_merges_flag_is_passed(monkeypatch, repo):
    ...
    monkeypatch.setattr("feishu_hub.git_log.subprocess.run", spy_run)
...
def test_no_git_binary_graceful(monkeypatch, repo):
    ...
    monkeypatch.setattr("feishu_hub.git_log.subprocess.run", fake_run)
