...
from feishu_hub import onboarding


def _mock_identity_ready():
    """构造一个 is_ready=True 的 Identity。"""
    from feishu_hub.identity import Identity
    return Identity(
...
def test_create_welcome_tasks_when_ready(capsys):
    with patch("feishu_hub.onboarding.current_identity", return_value=_mock_identity_ready()), \
         patch("feishu_hub.onboarding.task_writer") as tw:
    ...
    for call in tw.get_or_create_for_session.call_args_list:
        ...
        assert call.kwargs["agent"] == "feishu_hub"


def test_skips_when_identity_not_ready(capsys):
    with patch("feishu_hub.onboarding.current_identity", return_value=_mock_identity_not_ready()), \
         patch("feishu_hub.onboarding.task_writer") as tw:
