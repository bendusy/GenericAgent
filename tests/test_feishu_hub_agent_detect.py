...
from feishu_hub.agent_detect import (
    AGENTS,
    AgentSpec,
    DetectResult,
    describe,
...
def test_detect_all_when_all_present():
    ...
    with patch("feishu_hub.agent_detect.shutil.which", side_effect=fake_paths.get):
        results = detect_all()
...
def test_detect_all_when_only_cc_present():
    ...
    with patch("feishu_hub.agent_detect.shutil.which", side_effect=fake_which):
        results = detect_all()
...
def test_skip_explicitly_excluded():
    ...
    with patch("feishu_hub.agent_detect.shutil.which", return_value="/usr/bin/anything"):
