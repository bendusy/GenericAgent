...
规则：``feishu_hub/*`` 除 ``llm_summary.py`` 外，不得引用 GA 内部模块。
覆盖两层：
...
PKG_ROOT = Path(__file__).resolve().parent.parent / "feishu_hub"
...
def test_no_forbidden_imports(path: Path):
    ...
    assert all_imports == set(), (
        ...
        f"Only feishu_hub/llm_summary.py may reference GA, and only lazily."
    )
    assert dyn == set(), (
