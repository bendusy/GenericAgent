"""feishu_hub.identity 测试：mock lark-cli subprocess，验证 profile/auth 解析 + fallback 顺序。"""
...
from feishu_hub import identity as ident_mod
from feishu_hub.identity import (
    Identity,
    current_identity,
    list_profiles,
...
def test_resolve_user_open_id_none_when_all_empty(monkeypatch):
    ...
    with patch.object(ident_mod, "_run_lark_cli", return_value=None):
        ...
        with patch("feishu_hub.config.load", return_value={}):
