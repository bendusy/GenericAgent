"""feishu_hub.hooks_merge 单测：模板渲染 + 幂等合并 + 原子写。"""
import json
from pathlib import Path

import pytest

from feishu_hub import hooks_merge


HOOK_SCRIPT = "/Users/ben/.feishu_hub/bin/agent-stop-notify.sh"
...
def test_merge_into_empty_file(tmp_path):
    ...
    assert merged["hooks"]["Stop"][0]["matcher"] == "*"
...
def test_merge_is_idempotent(tmp_path):
    ...
    stop_hooks = merged2["hooks"]["Stop"][0]["hooks"]
...
def test_merge_appends_when_existing_hook_different(tmp_path):
    ...
    frag = hooks_merge.render_template("claude_code_settings.json.tmpl",
                                        hook_script=HOOK_SCRIPT)
    merged = hooks_merge.merge_stop_hook(p, frag)
    stop_hooks = merged["hooks"]["Stop"][0]["hooks"]
...
def test_merge_adds_new_matcher_bucket(tmp_path):
    p = tmp_path / "settings.json"
    p.write_text(json.dumps({
        "hooks": {"Stop": [{"matcher": "Read", "hooks": []}]}
    }), encoding="utf-8")
    frag = hooks_merge.render_template("claude_code_settings.json.tmpl",
                                        hook_script=HOOK_SCRIPT)
    merged = hooks_merge.merge_stop_hook(p, frag)
    matchers = [b["matcher"] for b in merged["hooks"]["Stop"]]
...
def test_apply_template_end_to_end(tmp_path):
    ...
    assert data["hooks"]["Stop"][0]["hooks"][0]["command"].endswith(HOOK_SCRIPT)
